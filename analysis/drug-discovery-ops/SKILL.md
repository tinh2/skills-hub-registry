...
If arguments are provided, use them to focus the analysis (e.g., specific therapeutic areas, pipeline stages, or compound libraries). If no arguments, scan the current project for drug discovery infrastructure, screening workflows, and pipeline management systems.
...
Identify drug discovery platforms:
...
Evaluate hit confirmation processes:
...
- Intellectual property (IP) freedom-to-operate assessment
- Chemical tractability evaluation

Step 3.2 -- Lead Optimization

Assess optimization workflows:
- Design-make-test-analyze (DMTA) cycle tracking
...
Evaluate ADMET model performance:
...
- Comparison to experimental data (predicted vs. measured concordance)

Step 4.3 -- Integration and Decision Support

Assess model integration:
...
- Alerts for liability flags (hERG > threshold, CYP inhibition, reactive metabolites)
