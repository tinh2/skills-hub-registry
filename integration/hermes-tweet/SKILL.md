...
- Keep `HERMES_TWEET_ENABLE_ACTIONS=false` for research, monitoring, support, and unattended sessions.
- Enable actions only for sessions where the user explicitly asks to post, reply, send DMs, follow, manage webhooks, manage monitors, or change media.
...
3. Use `tweet_action` only after confirming the request is write-like and action gating is enabled.
4. Do not call private or write-like routes through `tweet_read`.
5. Do not enable actions for cron or gateway jobs unless the workflow requires writes.
