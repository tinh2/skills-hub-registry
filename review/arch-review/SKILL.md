...
version: 1.0.0
category: review
  Architect-level design review and implementation validation with domain consistency analysis.
  Run BEFORE coding to get design feedback on a story, spec, RFC, or feature description.
  Run AFTER coding to validate a branch against requirements.
  Trigger this skill whenever you have a story to review, a spec to validate, a branch to audit,
  or any time you want an architect's opinion before or after implementation.
platforms:
  - CLAUDE_CODE
---
...
- If the project has a single monolithic service file handling all data operations for multiple domains, **FLAG THIS as Critical risk**. A single file that serves as a god object for all business logic will accumulate disproportionate churn (often 20%+ of all fix commits) and become unmaintainable.
- Service files should be organized by business domain (e.g., BookingService, UserService, PaymentService), not by technology layer (e.g., one giant DatabaseService).
- Each service should own a single domain's CRUD operations and business logic.
- No service file should handle more than 3 closely-related collections/tables.
- Recommend domain-split services from Day 1 — retrofitting this later is extremely expensive.

### DATA PRIVACY CHECK

For every data model in the story, evaluate public vs private data separation:
- Identify which fields contain PII (name, email, phone, address, payment info, location, IP addresses, device identifiers).
- If the model will be read by other users (e.g., profiles, reviews, listings), ensure a public projection exists that excludes PII fields.
...
- This prevents expensive late-breaking migrations to separate public and private data.

### INFRASTRUCTURE CHECK

If the story involves Docker, shell scripts, CI/CD, cloud infrastructure, or deployment changes:

**Container & Orchestration:**
- Verify docker-compose volume mounts target paths the application actually reads from.
- Verify config file delivery matches code's config discovery mechanism (default paths vs env vars vs CLI flags).
- Verify image references use full registry paths (e.g., `ghcr.io/org/image`), not bare names that default to Docker Hub.
- Check that exposed ports do not conflict with other services in the compose stack.
- Verify health checks exist and target endpoints the application actually serves.
- Check resource limits (memory, CPU) are set for production deployments.

**Shell Script Portability:**
- Check `sed -i` (needs `''` on macOS), `readlink -f` (not available on macOS), `date` flags, `mktemp` patterns.
- Verify shebang lines match the intended shell (`#!/bin/bash` vs `#!/bin/sh`).
- Check for bashisms in scripts declared as `/bin/sh`.

**Path & Config Integrity:**
- After any directory reorganization, verify all relative paths (`$SCRIPT_DIR`, `../`, `./`) still resolve correctly.
- Flag `tar -C` or `cp` commands that reference old directory structures.
- Verify environment variable names are consistent between `.env` files, docker-compose, and application code.

**Variable Scoping:**
- Verify shell variables reference the correct entity (e.g., `$TARGET_ID` vs `$CALLER_ID` — using one where the other was intended).
- Check for variable name collisions in multi-service setups.

**CI/CD:**
- Verify pipeline steps run in the correct order (test before deploy).
- Check that secrets are referenced by the correct names in the CI environment.
- Verify artifact paths match between build and deploy stages.
...
- Verify a migration system exists in the project (e.g., node-pg-migrate, knex, flyway, alembic, Django migrations, Prisma migrate, sequelize-cli, goose, Liquibase). If none exists, flag as **Critical risk** and recommend adopting one BEFORE implementing the schema change.
- Schema changes MUST be tracked as numbered, reversible migrations — not raw DDL edits to a monolithic schema file.
- Each migration must include an up AND down function (or equivalent rollback strategy).
- Flag any project that modifies schema via direct DDL file edits (e.g., editing a monolithic `tables.sql` or `schema.sql`) without a migration tool. This leads to production schema drift with no rollback capability.
- For greenfield projects: recommend setting up migrations as part of the first schema story.
- For existing projects without migrations: recommend a "baseline migration" that captures current schema state before adding new changes.
...
- Missing observability (logging, metrics, alerts).
...
- Serialization — toJson/fromJson/ORM mappings/Firestore mappings cover all new fields.
- API consistency — new endpoints match what the frontend calls, shapes match.
- State management — new providers/stores are defined and used correctly.
...
- Is the layering correct (controller -> service -> repository)?
...
- Docker Compose: Verify image references use full registry paths. Verify volume mount target paths exist in the container and match where the application actually reads config. Check health checks and resource limits.
- Config path assumptions: If code reads config from a default path, verify the deployment mounts config there — not via env var overrides the app ignores. Flag mismatches between code's config discovery and deployment's config delivery.
- Shell script portability: Check `sed -i` (needs `''` on macOS), `readlink -f` (not on macOS), `date` flags. Run `bash -n` syntax validation on all `.sh` files.
- Path reference integrity: After any directory reorganization, verify all relative paths (`$SCRIPT_DIR`, `../`, `./`) still resolve correctly. Flag `tar -C` or `cp` commands that reference old directory structures.
- Variable scoping: Verify shell variables reference the correct entity. Check for variable name collisions across services.
- CI/CD: Verify pipeline steps, secret references, and artifact paths are consistent.
